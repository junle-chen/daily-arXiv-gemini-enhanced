...
TL;DR: AskDB是一个基于Gemini 2的大语言模型驱动的智能体，旨在通过自然语言支持SQL数据库的数据分析和管理任务。
...
Method: AskDB集成了动态模式感知提示机制和任务分解框架，以有效利用数据库元数据并规划执行多步骤操作。

Result: 在Text-to-SQL基准测试和DBA任务评估中表现出色，展示了其在分析和管理场景中的潜力。

Conclusion: AskDB作为一个统一的智能体，为关系数据库系统提供直观易用的体验。

Abstract: 与关系数据库交互对不同专业水平的用户来说仍然具有挑战性，特别是在编写复杂的分析查询或执行管理任务时。现有的系统通常只关注自然语言查询或数据库管理的狭窄方面，缺乏一个统一和智能的界面来进行通用数据库交互。我们介绍了AskDB，这是一个由大型语言模型驱动的智能体，旨在通过自然语言支持SQL数据库上的数据分析和管理操作，从而弥合这一差距。AskDB构建于Gemini 2之上，集成了两项关键创新：一种动态的模式感知提示机制，可以有效地整合数据库元数据；以及一个任务分解框架，使智能体能够规划和执行多步骤操作。这些能力使AskDB能够自主调试派生的SQL，通过实时网络搜索检索上下文信息，并自适应地完善其响应。我们在一个广泛使用的Text-to-SQL基准测试和一个精心策划的DBA任务集上评估了AskDB，证明了其在分析和管理场景中的强大性能。我们的结果突显了AskDB作为关系数据库系统的统一和智能代理的潜力，为最终用户提供了直观和可访问的体验。
