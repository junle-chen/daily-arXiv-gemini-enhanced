...
TL;DR: 该论文提出了一种建模和评估数据共享管道中能耗的方法，并识别了跨管道共享阶段的重用潜力。


<details>
  <summary>Details</summary>
Motivation: 当前数据共享管道的能效问题受关注不足。

Method: 提出了一种新的方法来建模和评估数据共享管道中不同执行配置的能耗，并识别共享阶段的重用潜力。

Result: 通过仿真实验验证了该方法，揭示了跨组织管道优化的潜力。

Conclusion: 该研究为节能执行策略奠定了基础。

Abstract: 数据共享管道涉及一系列阶段，这些阶段应用基于策略的数据转换，以实现组织之间安全有效的数据交换。尽管有许多工具和平台可以管理这些管道中的治理和执行，但数据交换中的能源效率却很少受到关注。本文介绍了一种新颖的方法，用于建模和估计数据共享管道中不同执行配置的能源消耗。此外，该方法还识别了大型数据共享联盟中减少能源的关键——跨管道共享阶段的重用潜力。我们通过仿真实验验证了该方法，揭示了跨组织管道优化的良好潜力，并为节能执行策略奠定了基础。
